  ...
  (t/testing "test public API"
    ...
    (let [required #{:db-uri}
      ...
      (sut/start '("-h") {:cli-spec cli-spec,
                          ...
                          :ok-fn capture-ok})
      (t/is (true? (-> @result :ok :show-help?)) "Expect the ok fn to capture the help result")
      (t/is (not (contains-errors? (-> @result :ok :summary)))))
    ...
    (let [required #{:db-uri :port}
      ...
      (sut/start '("--port" "8080" "-l" "some-string") {:cli-spec cli-spec,
                                                        ...
                                                        :ok-fn capture-ok})
      (t/is (nil? (-> @result :ok)))
      (t/is (= #{:db-uri} (-> @result :notok :missing-args)))
      (t/is (= :missing-required-args  (-> @result :notok :error))
    ...
    (let [required #{:db-uri}
      ...
      (t/is (= {:db-uri "datomic-uri" :port 8080} @result)
